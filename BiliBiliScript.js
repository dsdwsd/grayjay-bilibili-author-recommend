...
const DYNAMIC_FEED_CACHE_TTL = 2 * 60 * 60 * 1000; // 2小时缓存
...
function _refreshDynamicFeedCache() {
    try {
        ...
        if (!json || json.length === 0) {
            log("BiliBili log: dynamic feed response empty");
            return null;
        }
        ...
        if (!response.data || !response.data.items) {
            log("BiliBili log: dynamic feed no data.items");
            return null;
        }
        // 按作者mid分组
        const items_by_mid = new Map();
        ...
        log("BiliBili log: _refreshDynamicFeedCache error: " + (e?.message || String(e)));
        return null;
    }
}
            ...
            const platform_video_details_def = {
                ...
                getContentRecommendations: function () {
                    ...
                    return new VideoPager(video_info.data.Related.filter((video) => {
                        return video.owner.mid === owner_mid;
                    }).map((video) => {
                        return new PlatformVideo({
